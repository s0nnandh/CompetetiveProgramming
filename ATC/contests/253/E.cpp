...
void solve(){
    int n,m,K;
    std :: cin >> n >> m >> K;
    std :: vector< std :: vector<ModInt>> dp(n, std :: vector<ModInt> (m + 1));
    std :: vector<ModInt> pre(m + 1); 
    for(int i = 1;i < m + 1;++i)dp[0][i] = 1;
    for(int i = 1;i < n;++i){
        for(int k = 1;k <= m;++k)pre[k] = pre[k - 1] + dp[i - 1][k];
        for(int j = 1;j <= m;++j){
            int l = std :: max(0,j - K),r = std :: min(m + 1,j + K);
            dp[i][j] += pre[l] + pre[m] - pre[r - 1];
