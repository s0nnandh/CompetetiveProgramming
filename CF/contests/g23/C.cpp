...
void solve() {
    ...
    vi v(n);
    for(int &i : v)cin >> i;
    vector<pi> points, moves;
    points.push_back({0, 0});
    for(int i = 1;i <= n;++i) {
        int mx = max(points[i - 1].first, v[i - 1]);
        points.push_back({mx, mx == v[i - 1] ? i : points[i].second});
    }
    for(int i = 1;i <= n;++i) {
        if(points[i].first > v[i - 1]) {
            moves.push_back({points[i].first - v[i - 1], i});
        }
    }
    sort(all(moves));
    int r = 0;
    vi ans;
    for(int i = 1;i <= n;++i) {
        if(r < (int)moves.size() && moves[r].first <= i)ans.push_back(moves[r++].second);
