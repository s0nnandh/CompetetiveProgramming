...
void solve(){
    ...
    int c = 1;
    int l = 2,r = 2 * n;
    while(l <= r)cnt[l++] = cnt[r--] = c++;
    vector<int> div(2 * n  + 1);
    for(int i = 2;i <= 2 * n;++i){
        for(int j = i;j <= 2 * n;j += i)div[j]++;
    }
    ll x = mpow(2 * n - 1,2),ans = 0,k = 2 * n - 1;
    for(int i = 2;i <= 2 * n;++i){
        ll me = mmul(cnt[i],msub(x,mmul(k - div[i],k - div[i])));
        // dbg(me,i,div[i]);
        ans = madd(ans,me);
    }
    cout << mmul(ans,minv(x)) << '\n';
}
